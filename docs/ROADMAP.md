...
> **状态快照**：2026-09-20 —— M4-2 已收官（508）；M4-2.5 容量与多模态已收官（511→558，v32、批次 17）；**TUI 界面批第一阶段已收官（558→598 四门全绿，v33、D52/批次 18、真终端走查矩阵 10/10）**——raw-mode 前置小批 T0–T8 全落地（键盘菜单/Esc 取消三件套/流式重绘/Alt+V/@ 补全 +#L/compact 指示）。**下一批 = T9 选型 spike（TUI 批阶段二框架化的入口——Ink / pi-tui / 自写三选一），随后 M4.5 按排队表顺位**。

## 排队中（按执行顺序——批号是里程碑编号非序号，插队以本表顺序为准）

| 顺序 | 批次 | 内容 | 状态 | 方案/出处 |
|---|---|---|---|---|
| 1 | **TUI 界面批**（原 M5 首项候选，2026-09-19 用户拍板提前） | ~~前置小批（raw-mode）~~ **✅ 第一阶段已收官（2026-09-20，`plans/2026-09-20-m5-tui-batch.md` T0–T8：558→598 四门全绿，v33 + 批次 18，走查矩阵 10/10）** → **TUI 框架化**：kimi-code 形态完整界面（全屏接管、流式重绘、编辑器/picker/状态栏 + **Markdown 复杂版渲染（语法高亮+表格——自零依赖第 1 层升格，2026-09-19 用户拍板并入）**；Ink / pi-tui / 自写三选一——**T9 选型 spike 为下一批**，随后框架化方案书立项） | 阶段二待立项（框架化方案书随 T9 交付） | 本文件「M5 / 独立里程碑候选」节 |
...
- **TUI 框架化（已升为排队第 1 位——2026-09-19 用户拍板提前：主要功能基本完工、界面优先；技术前置已成熟）**：kimi-code 形态完整界面（全屏接管、流式重绘、编辑器/picker/状态栏；**含 Markdown 复杂版渲染（高亮+表格，自 V.2 池并入）**；Ink / pi-tui / 自写三选一）。技术前置已成熟（D45 旁路通道 ✅ + CommandUi 接缝 ✅ + **raw-mode 模态接管制 D52 ✅——2026-09-20 第一阶段收官**：键盘菜单/Esc 取消/流式重绘/Alt+V/@ 补全 +#L 全链落地，滚动流形态已产品化），唯一缺选型 spike（T9，一周）。
