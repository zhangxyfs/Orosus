...
> **状态快照**：2026-09-20 —— M4-2 已收官（508）；M4-2.5 容量与多模态已收官（511→558，v32、批次 17）；**TUI 界面批第一阶段已收官（558→600 四门全绿——收官 598 + 当日用户实测四项修整 +2，v33、D52/批次 18、真终端走查矩阵 10/10）**——raw-mode 前置小批 T0–T8 全落地（键盘菜单/Esc 取消三件套/流式重绘/Alt+V/@ 补全 +#L/compact 指示）。**下一批 = T9 定向验证 spike（TUI 批阶段二框架化的入口——方向已拍板 2026-09-20：pi-tui 方式，自写薄层·抄架构不引包·零 npm 依赖，用户认可六家调研 P1 建议；spike 出实测验证 + 框架化方案书），随后 M4.5 按排队表顺位**。

## 排队中（按执行顺序——批号是里程碑编号非序号，插队以本表顺序为准）

| 顺序 | 批次 | 内容 | 状态 | 方案/出处 |
|---|---|---|---|---|
| 1 | **TUI 界面批**（原 M5 首项候选，2026-09-19 用户拍板提前） | ~~前置小批（raw-mode）~~ **✅ 第一阶段已收官（2026-09-20，`plans/2026-09-20-m5-tui-batch.md` T0–T8：558→600 四门全绿，v33 + 批次 18，走查矩阵 10/10）** → **TUI 框架化**：kimi-code 形态完整界面（全屏接管、流式重绘、编辑器/picker/状态栏 + **Markdown 复杂版渲染（语法高亮+表格——自零依赖第 1 层升格，2026-09-19 用户拍板并入）**；**引擎方向已拍板（2026-09-20）：pi-tui 方式——自写薄层·抄架构不引包·零 npm 依赖（原「三选一」由本拍板定向取代）——T9 定向验证 spike 为下一批**，随后框架化方案书立项） | 阶段二待立项（框架化方案书随 T9 交付） | 本文件「M5 / 独立里程碑候选」节 |
