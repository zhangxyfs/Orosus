...
## 4. Step 3：过信任门（项目级模块必看）

项目级模块第一次加载会被拦下（§8.5，防 MCPoison 式投毒）：

```
untrusted（项目级模块未确认——运行 orosus module trust note 后重启生效）
```

- 确认一次登记进 `~/.orosus/trust.json`（内容 hash）；
- 之后**改了入口文件**，hash 对不上 → 重新确认。这是特性不是 bug：项目里的模块代码变了，
  你应该再看一眼。
- 用户级（`~/.orosus/modules/`）恒免确认——你自己放的东西自己负责。
