...
// M4-2.5 T5：五家内置 provider 同款图片映射钉（glm 走 anthropic 线缆）
const { mkdtempSync, rmSync, writeFileSync } = await import("node:fs");
const { tmpdir } = await import("node:os");
const { join } = await import("node:path");
const { toAnthropicMessages } = await import("./translate.ts");
...
describe("toAnthropicMessages 图片映射（M4-2.5 T5 五家同款）", () => {
  it("image part → base64 source block", () => {
    const dir = mkdtempSync(join(tmpdir(), "orosus-t5d-"));
    const p = join(dir, "shot.png");
    writeFileSync(p, Buffer.from([9, 9]));
    ...
    expect((out[0]!.content[0]!["source"] as { data: string }).data).toBe(Buffer.from([9, 9]).toString("base64"));
