...
export function renderBlock(
	...
	switch (t.type) {
		case "heading": {
			...
			// 样式上下文（T6 哨兵）：外层包裹不动，内层收尾后贴回前缀——标题色断不了
			const outer = h.depth <= 2
				? (text: string): string => theme.bold(theme.fg("accent", text))
				: (text: string): string => theme.bold(theme.fg("info", text));
			const headingCtx: InlineStyleContext = { applyText: (text) => text, stylePrefix: stylePrefixOf(outer) };
		...
		case "paragraph":
			out.push(inlineTokens((t as Tokens.Paragraph).tokens ?? [], ctx));
			out.push("");
			break;
		case "list": {
		...
		case "blockquote": {
			// 悬挂缩进（mdpipe 批 T3）+ 样式上下文（T6）：子块按 width-2 渲染 → 逐行按 width-2
			// 折行 → 每条物理行贴 ▎ 前缀（先折行后贴前缀——折行知晓前缀占用，续行不丢引用符）；
			// 引用色经 ctx.stylePrefix 在行内内层收尾后恢复。
			const quoteCtx: InlineStyleContext = {
				applyText: (text) => text,
				stylePrefix: stylePrefixOf((text) => theme.fg("muted", text)),
			};
			const sub: string[] = [];
